...
# Obtener ruta del archivo de facturas
excel_file_path = config_manager.get_file_path('facturas')
#print("-->")
...
#    cnx.commit()
    ...
    SQL = "delete from facturas_his where fecha > '2026-01-01'; ALTER TABLE facturas_his AUTO_INCREMENT = 1;"    
    SQL = SQL + "insert into facturas_his (FECHA, EMISOR, PRECIO_PORC, VALOR_NOMINAL, VALOR_EFECTIVO, EMISION, VENCIMIENTO, RENDIMIENTO, PROCEDENCIA, OBSERVACIONES) SELECT `FECHA`, `EMISOR`, `PRECIO %%`, `VALOR NOMINAL (USD)`, `VALOR EFECTIVO (USD)`, `F. EMISION`, `F. VENCIMIENTO`, `RENDIMIENTO %%`, `PROCEDENCIA`, `OBSERVACIONES` FROM `facturas_his_jao` where emisor is not null and fecha > (SELECT IFNULL(MAX(fecha), '2100-01-01') FROM facturas_his)"    
