...
# Obtener ruta del archivo de genéricos
excel_file_path = config_manager.get_file_path('genericos')
#print("-->")
...
#    cnx.commit()
    ...
    SQL = "delete from genericos_his where fecha > '2026-01-01'; ALTER TABLE genericos_his AUTO_INCREMENT = 1;"    
    SQL = SQL + "insert into genericos_his (FECHA, EMISOR, PRECIO_PORC, RENDIMIENTO, PLAZO_DIAS, INTERES, VALOR_NOMINAL, VALOR_EFECTIVO, EMISION, VENCIMIENTO, PROCEDENCIA, TITULO, MERCADO) SELECT `FECHA`, `EMISOR`, `PRECIO %%`, `RENDIMIENTO %%`, `PLAZO POR VENCER (DÍAS)`, `INTERÉS %%`, `VALOR NOMINAL (USD)`, `VALOR EFECTIVO (USD)`, `FECHA DE EMISIóN`, `FECHA VENCIMIENTO`, `PROCEDENCIA`, `TíTULO`,`TIPO DE MERCADO` FROM `genericos_his_jao` where emisor is not null and fecha > (SELECT IFNULL(MAX(fecha), '2100-01-01') FROM genericos_his)"    
